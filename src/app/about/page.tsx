...
export default function AboutPage() {
  return (
    <main style={{ background: '#0a0a0a', minHeight: '100vh', paddingTop: '64px' }}>
      {/* Hero */}
      <section style={{ position: 'relative', height: '60vh', minHeight: '300px', overflow: 'hidden', display: 'flex', alignItems: 'flex-end', background: '#050505' }}>
        ...
        <div className="section-inner" style={{ position: 'relative', zIndex: 2, width: '100%', paddingBottom: '48px' }}>
          ...
          <h1 style={{ fontFamily: 'Outfit', fontSize: 'clamp(1.8rem, 5vw, 4rem)', fontWeight: 900, letterSpacing: '-0.02em', textTransform: 'uppercase', color: 'white' }}>
            Windows to a<br />Brighter Tomorrow
          </h1>
        </div>
      </section>

      {/* Stats — 2col on mobile, 4col on desktop */}
      <section style={{ background: '#141414', padding: '0' }}>
        <div className="stats-grid" style={{ maxWidth: '1400px', margin: '0 auto' }}>
          {stats.map((s, i) => (
            <div key={s.label} style={{ padding: '40px 24px', textAlign: 'center', borderRight: i < 3 ? '1px solid rgba(255,255,255,0.07)' : 'none' }}>
              <p style={{ fontFamily: 'Outfit', fontSize: 'clamp(1.8rem, 5vw, 2.5rem)', fontWeight: 900, color: '#c8a96e', marginBottom: '8px' }}>{s.number}</p>
              <p style={{ fontSize: '10px', letterSpacing: '0.15em', color: 'rgba(255,255,255,0.45)', textTransform: 'uppercase', fontWeight: 600 }}>{s.label}</p>
            </div>
          ))}
        </div>
      </section>

      {/* Mission — 2col → 1col on mobile */}
        ...
        <div className="section-inner">
          <div className="two-col">
            <div>
              ...
              <h2 style={{ fontFamily: 'Outfit', fontSize: 'clamp(1.6rem, 3vw, 2.8rem)', fontWeight: 900, letterSpacing: '-0.02em', textTransform: 'uppercase', color: 'white', lineHeight: 1.1, marginBottom: '24px' }}>
                Engineering Light Into Every Home
              </h2>
              <p style={{ fontSize: '15px', color: 'rgba(255,255,255,0.5)', lineHeight: 1.9, marginBottom: '18px' }}>
              ...
              <p style={{ fontSize: '15px', color: 'rgba(255,255,255,0.5)', lineHeight: 1.9, marginBottom: '32px' }}>
